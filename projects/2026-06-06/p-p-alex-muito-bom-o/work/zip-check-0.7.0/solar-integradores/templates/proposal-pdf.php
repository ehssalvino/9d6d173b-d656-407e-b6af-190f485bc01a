...
<style>
    ...
    *{box-sizing:border-box}
    ...
    .chart{height:60mm;border:1px solid #c7d6e0;padding:4mm 3mm 7mm;vertical-align:bottom}
    .chart td{height:48mm;vertical-align:bottom;text-align:center;font-size:7pt}
...
<section class="page">
    ...
    <table class="data">
        ...
        <tr><td>Módulos</td><td><?php echo esc_html(($equipment['module'] ?? 'A definir') . ' - ' . ($equipment['module_power_w'] ?? 0) . ' W'); ?></td><td><?php echo esc_html(($equipment['module_count'] ?? 0) . ' módulos / ' . $number($equipment['installed_kwp'] ?? 0) . ' kWp'); ?></td></tr>
        <tr><td>Inversor(es)</td><td><?php echo esc_html($equipment['inverter_model'] ?? 'Inversor on-grid'); ?></td><td><?php echo esc_html(($equipment['inverter_count'] ?? 0) . ' × ' . $number($equipment['inverter_unit_power_kw'] ?? 0) . ' kW'); ?></td></tr>
        <tr><td>Relação DC/AC</td><td><?php echo esc_html($equipment['inverter'] ?? 'A definir'); ?></td><td><?php echo esc_html($number($equipment['inverter_overload_percent'] ?? 0) . '% overload'); ?></td></tr>
    ...
    <table class="chart"><tr>
        ...
        <td>
            <div class="bar-consumption" style="height:<?php echo esc_attr(max(2, ($month['consumption'] / $max_generation) * 42)); ?>mm"></div>
            <div class="bar-generation" style="height:<?php echo esc_attr(max(2, ($month['generation'] / $max_generation) * 42)); ?>mm"></div><br>
