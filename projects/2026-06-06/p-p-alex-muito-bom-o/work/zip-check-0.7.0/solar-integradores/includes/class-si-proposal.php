...
final class SI_Proposal
{
    ...
    private static function select_equipment(array $result, array $settings)
    {
        $kwp = (float) ($result['system_kwp'] ?? 0);
        $module_count = (int) ($result['module_count'] ?? 0);
        $module_power = (int) ($result['module_power_w'] ?? 0);
        $installed_kwp = (float) ($result['installed_system_kwp'] ?? (($module_count * $module_power) / 1000));
        $inverter_count = max(1, (int) ($result['inverter_count'] ?? 1));
        $inverter_unit_power = (float) ($result['inverter_unit_power_kw'] ?? 0);
        $inverter_total_power = (float) ($result['inverter_total_power_kw'] ?? 0);
        $overload = (float) ($result['inverter_overload_percent'] ?? 0);
        $inverter_description = sprintf(
            '%dx inversor de %s kW (%s kW AC total)',
            $inverter_count,
            number_format_i18n($inverter_unit_power, 2),
            number_format_i18n($inverter_total_power, 2)
        );
        $selected = array(
            ...
            'inverter' => $inverter_description,
            'inverter_model' => sanitize_text_field($settings['default_inverter_model'] ?? 'Inversor on-grid'),
            'inverter_count' => $inverter_count,
            'inverter_unit_power_kw' => round($inverter_unit_power, 2),
            'inverter_total_power_kw' => round($inverter_total_power, 2),
            'inverter_overload_percent' => round($overload, 2),
            'quoted_cost' => null,
        );
        ...
        foreach ($lines as $line) {
            ...
            $selected = array(
                ...
                'installed_kwp' => round($installed_kwp, 2),
                'inverter' => $inverter_description,
                'inverter_model' => sanitize_text_field($columns[4]),
                'inverter_count' => $inverter_count,
                'inverter_unit_power_kw' => round($inverter_unit_power, 2),
                'inverter_total_power_kw' => round($inverter_total_power, 2),
                'inverter_overload_percent' => round($overload, 2),
