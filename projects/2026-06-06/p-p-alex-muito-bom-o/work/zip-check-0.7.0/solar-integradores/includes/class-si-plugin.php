...
final class SI_Plugin
{
    ...
    public static function activate()
    {
        ...
        if (!get_option('si_settings')) {
            add_option('si_settings', array(
                ...
                'inverter_sizes_kw' => SI_Calculator::default_inverter_sizes(),
                'default_tariff' => 1.00,
                ...
                'default_inverter_model' => 'Inversor on-grid',
                'supplier_catalog' => '',
            ));
        } else {
            ...
            if (
                empty($settings['default_inverter_model'])
                || $settings['default_inverter_model'] === 'Inversor compatível com o projeto'
            ) {
                $settings['default_inverter_model'] = 'Inversor on-grid';
            }
    ...
    public function ajax_calculate()
    {
        ...
        $input['inverter_overload_percent'] = $settings['inverter_overload_percent'];
        $input['inverter_sizes_kw'] = $settings['inverter_sizes_kw'];
    ...
    public function render_public_quote()
    {
        ...
        if (empty($result['proposal']['equipment']['inverter_unit_power_kw'])) {
            $result['proposal'] = SI_Proposal::build($input, $result, $this->settings());
        }
        status_header(200);
        nocache_headers();
    ...
    public function sanitize_settings($input)
    {
        ...
        return array(
            ...
            'inverter_overload_percent' => min(60, max(0, (float) str_replace(',', '.', $input['inverter_overload_percent'] ?? 30))),
            'inverter_sizes_kw' => preg_replace('/[^0-9.,;\s]/', '', (string) ($input['inverter_sizes_kw'] ?? SI_Calculator::default_inverter_sizes())),
