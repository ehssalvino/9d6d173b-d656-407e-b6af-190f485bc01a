...
final class SI_Plugin
{
    ...
    public function ajax_request_quote()
    {
        ...
        $email_sent = $this->send_quote_emails($name, $email, $phone, $quote_url, $result, $channel);

        $share_text = sprintf(
            __('Olá, %1$s. Sua estimativa solar de %2$s kWp, com investimento aproximado de R$ %3$s, está disponível em %4$s', 'solar-integradores'),
            $name,
        ...
        wp_send_json_success(array(
            'message' => __('Solicitação registrada. Seu resumo de orçamento está pronto.', 'solar-integradores'),
            'quote_url' => $quote_url,
    ...
    private function send_quote_emails($name, $email, $phone, $quote_url, array $result, $channel)
    {
        ...
        if (in_array($channel, array('email', 'both'), true)) {
            $customer_sent = wp_mail($email, $subject, $message);
        }
