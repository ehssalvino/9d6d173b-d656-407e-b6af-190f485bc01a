...
from .string_design import design_string_arrangement, fallback_string_design
...
def _calculated_values(workbook) -> dict[str, Any]:
    ...
    try:
        string_design = design_string_arrangement(inp, strict=False)
    except Exception:
        string_design = fallback_string_design(inp)
    design_strings = string_design.string_count or (2 if module_qty >= 10 else 1)
    modules_by_string = string_design.modules_by_string or [
        int((module_qty + design_strings - 1) // design_strings) if module_qty else 0
    ]
    modules_per_string = modules_by_string[0] if len(set(modules_by_string)) == 1 else max(modules_by_string)
    if n("INV_STRINGS") is not None:
        values["{{INV_STRINGS}}"] = n("INV_STRINGS")
    values["{{STRING_QTD}}"] = design_strings
    values["{{STRING_ARRANJO}}"] = "; ".join(
        f"CC {item.label}: {item.modules} módulos" for item in string_design.strings
    )
    ...
    values["{{DV_CC_V}}"] = (
        n("MOD_IMP") * n("R_CC_OHMKM") * (n("DIST_CC_M") / 1000)
...
def _capture_google_maps(address: str, output: Path) -> bool:
    edge = _find_edge()
    if edge is None or not address.strip():
        return False
    with tempfile.TemporaryDirectory() as profile:
        url = (
            "https://www.google.com/maps/place/"
            f"{quote_plus(address.strip())}/data=!3m1!1e3?hl=pt-BR"
        )
        command = [
            str(edge),
            "--headless",
        ...
        try:
            ...
            return False
    return output.is_file() and output.stat().st_size > 50_000


def _make_location_figure(latitude: Any, longitude: Any, address: str, output: Path) -> Path:
    if _capture_google_maps(address, output):
        return output
...
def _insert_location_figure(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        figure = _make_location_figure(
            latitude,
            longitude,
            address,
            Path(temp_dir) / "localizacao.png",
        )
        return _insert_picture_at_placeholder(docx_path, "{{FIG_LOCALIZACAO}}", figure, width=6.2)


def _expand_cc_rows(docx_path: Path, workbook) -> None:
    inputs = {
        workbook["Inputs"][f"A{row}"].value: workbook["Inputs"][f"B{row}"].value
        for row in range(2, workbook["Inputs"].max_row + 1)
    }
    module_qty = int(_number(inputs.get("MOD_QTD")) or 0)
    if module_qty <= 0:
        return

    try:
        string_design = design_string_arrangement(inputs, strict=False)
    except Exception:
        string_design = fallback_string_design(inputs)
    strings = string_design.strings
    if not strings:
        return
    string_count = string_design.string_count

    vmp = _number(inputs.get("MOD_VMP"))
    imp = _number(inputs.get("MOD_IMP"))
    resistance = _number(inputs.get("R_CC_OHMKM"))
    distance = _number(inputs.get("DIST_CC_M"))
    voltage_drop = (
        imp * resistance * (distance / 1000)
        if imp is not None and resistance is not None and distance is not None
        else None
    )
    ...
    table = next(
        (
            ...
            if any("Circuitos CC" in cell.text for cell in item.rows[0].cells)
        ),
        None,
    )
    ...
    for item in strings:
        ...
        nominal_voltage = vmp * modules if vmp is not None else None
        drop_ratio = _safe_div(voltage_drop, nominal_voltage)
        row_replacements = {
            "CC1": f"CC {item.label}",
            "{{VCC_NOM}}": _fmt(nominal_voltage),
            "{{ICC_MAX}}": _fmt(imp),
            "{{DV_CC_V}}": _fmt(voltage_drop),
            "{{DV_CC_PERC}}": _fmt(drop_ratio, "{{DV_CC_PERC}}"),
            "{{MODULOS_POR_STRING}}": str(modules),
            "{{STRING_QTD}}": str(string_count),
        }
        for paragraph in row.iter(W_P):
            _replace_in_paragraph(paragraph, row_replacements)
        for text_node in row.xpath(".//w:t"):
            if text_node.text and "M" in text_node.text and "string" in text_node.text.lower():
                text_node.text = text_node.text.replace(
                    text_node.text,
                    text_node.text.replace("{{STRING_QTD}}", str(string_count)).replace("{{MODULOS_POR_STRING}}", str(modules)),
                )
        first_cell_text = row.xpath("./w:tc[1]//w:t")
        if first_cell_text:
            first_cell_text[0].text = f"CC {item.label}"
...
def generate_memorial(excel: str | Path, template: str | Path, output: str | Path) -> GenerationResult:
    ...
    address = " ".join(
        str(input_values.get(key) or "").strip()
        for key in ("CLIENTE_ENDERECO_LOGRADOURO", "CLIENTE_BAIRRO_MUN_UF_CEP")
    ).strip()
    _expand_cc_rows(output_path, workbook)
    if address or (latitude not in (None, "") and longitude not in (None, "")):
        inserted = _insert_location_figure(output_path, latitude, longitude, address)
        if not inserted:
