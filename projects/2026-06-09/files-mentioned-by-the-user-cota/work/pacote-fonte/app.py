...
class App:
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(
            outer,
            text="Preencha os dados, revise os itens e gere a proposta em Word e PDF.",
            style="Sub.TLabel",
        ...
        ttk.Button(top, text="Salvar dados", command=self.save_json).pack(side="right")
        ttk.Button(top, text="Abrir dados", command=self.load_json).pack(side="right", padx=6)

        notebook = ttk.Notebook(outer)
        notebook.pack(fill="both", expand=True)
        self.general_tab = ttk.Frame(notebook, padding=12)
        self.technical_tab = ttk.Frame(notebook, padding=12)
        self.items_tab = ttk.Frame(notebook, padding=12)
        self.scope_tab = ttk.Frame(notebook, padding=12)
        notebook.add(self.general_tab, text="1. Cliente e proposta")
        notebook.add(self.technical_tab, text="2. Dados técnicos")
        notebook.add(self.items_tab, text="3. Itens e valores")
        notebook.add(self.scope_tab, text="4. Escopo e condições")
    ...
    def generate(self, with_pdf: bool) -> None:
        try:
            ...
            if with_pdf:
                generated.append(str(convert_to_pdf(destination)))
