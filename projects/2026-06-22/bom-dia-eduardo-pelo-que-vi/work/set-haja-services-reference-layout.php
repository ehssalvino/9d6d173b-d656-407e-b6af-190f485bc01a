...
$css = <<<'CSS'
<style>
#haja-servicos-principais{padding:72px 0;background:#fff}
#haja-servicos-principais .haja-servicos-grid{display:grid !important;grid-template-columns:repeat(4,1fr) !important;gap:18px !important;align-items:stretch !important;max-width:920px !important;margin:0 auto !important}
#haja-servicos-principais .haja-servico-card{box-sizing:border-box !important;display:flex !important;flex-direction:column !important;align-items:center !important;text-align:center !important;min-height:512px !important;padding:22px 12px 40px !important;border:1px solid #f4ab17 !important;background:#fff !important;color:#222 !important;text-decoration:none !important;box-shadow:none !important;border-radius:0 !important;overflow:visible !important}
#haja-servicos-principais .haja-icon-frame{position:relative !important;width:166px !important;height:76px !important;margin:8px auto 16px !important;display:flex !important;align-items:center !important;justify-content:center !important}
...
if ($start === false) {
    $start = strpos($content, '<section id="haja-servicos-principais">');
}
if ($start === false) {
    throw new RuntimeException('Services section start not found');
}

$altStart = strpos($content, '<section class="haja-band alt">', $start);
if ($altStart === false) {
    throw new RuntimeException('Next section not found');
}

$newContent = substr($content, 0, $start) . $services . substr($content, $altStart);
...
echo "Services section matched reference layout.\n";
