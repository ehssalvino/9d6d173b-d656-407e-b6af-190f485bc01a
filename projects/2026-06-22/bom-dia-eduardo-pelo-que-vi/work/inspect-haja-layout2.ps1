...
$userData = Join-Path (Resolve-Path '.') 'work\chrome-layout-profile-2'
$port = 9224
...
try {
  $tab = (Invoke-RestMethod "http://localhost:$port/json" | Where-Object { $_.url -like '*hajageracaosolar*' } | Select-Object -First 1)
