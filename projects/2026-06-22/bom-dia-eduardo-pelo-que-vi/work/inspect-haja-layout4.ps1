...
$userData = Join-Path (Resolve-Path '.') 'work\chrome-layout-profile-4'
$port = 9226
$url = 'http://localhost/hajageracaosolar/'
New-Item -ItemType Directory -Force -Path $userData | Out-Null
$proc = Start-Process -FilePath $chrome -ArgumentList @('--headless=new','--disable-gpu','--hide-scrollbars','--remote-allow-origins=*',"--remote-debugging-port=$port","--user-data-dir=$userData",'--window-size=1366,900',$url) -PassThru -WindowStyle Hidden
...
try {
  ...
  $ws.ConnectAsync([Uri]([string]$tab.webSocketDebuggerUrl), [Threading.CancellationToken]::None).Wait()
  $script:msgId = 0
